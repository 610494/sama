...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        ...
        # [關鍵]：輸入 x_norm 已經被正規化
        # [關鍵]：不再回傳 (output, adapter_output)，只回傳純粹的計算結果
        # [關鍵]：不再執行 x + ... 的殘差連接
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        # --- [關鍵改動 1] Student Adapter (並行) ---
        self.add_adapter = add_adapter
        self.adapter_dim = adapter_dim
        if add_adapter:
            print("Adding PARALLEL adapters (following Interspeech 2024 PGA)")
            # 我們為每個原始模組建立一個並行的 Adapter
            self.attn_adapter = Adapter(hidden_dim=n_state, bottleneck_dim=adapter_dim)
            self.mlp_adapter = Adapter(hidden_dim=n_state, bottleneck_dim=adapter_dim)
            
            if cross_attention:
                self.cross_attn_adapter = Adapter(hidden_dim=n_state, bottleneck_dim=adapter_dim)
            else:
                self.cross_attn_adapter = None
        # [REMOVED] 舊的單一 self.adapter 已經被移除了
    ...
    def forward(
        ...
        # 1. Teacher Block (TG-ASR) - 保持不變 (它本來就是一種 "順序" 結構)
        if self.add_gated_x_attn != 0: 
            x, gated_feats = self.apply_gated_x_attn_multi(x, xt_list)
            if collect_feats:
                collected_features['gated_x_attn'] = x # 收集 TG-ASR 模組的輸出

        # 2. Self-Attention Block - [關鍵改動 2: 改為並行]
        attn_residual = x
        x_norm = self.attn_ln(x)
        
        # 原始路徑
        attn_output = self.attn(x_norm, mask=mask, kv_cache=kv_cache)[0]
        
        if self.add_adapter:
            # 並行 Adapter 路徑
            attn_adapter_output = self.attn_adapter(x_norm) # 使用修改後的 Adapter
            
            # 並行融合：Residual + 原始輸出 + Adapter 輸出
            x = attn_residual + attn_output + attn_adapter_output 
            
            if collect_feats:
                collected_features['attn_adapter'] = attn_adapter_output # 收集純 Adapter 輸出
        else:
            # 原始融合
            x = attn_residual + attn_output

        # 3. Cross-Attention Block - [關鍵改動 3: 改為並行]
        if self.cross_attn:
            cross_attn_residual = x
            x_norm = self.cross_attn_ln(x)
            
            # 原始路徑
            cross_attn_output = self.cross_attn(x_norm, xa, kv_cache=kv_cache)[0]
            
            if self.add_adapter:
                # 並行 Adapter 路徑
                cross_attn_adapter_output = self.cross_attn_adapter(x_norm)
                
                # 並行融合
                x = cross_attn_residual + cross_attn_output + cross_attn_adapter_output
                
                if collect_feats:
                    collected_features['cross_attn_adapter'] = cross_attn_adapter_output
            else:
                # 原始融合
                x = cross_attn_residual + cross_attn_output
        
        # 4. MLP Block - [關鍵改動 4: 改為並行]
        mlp_residual = x
        ...
        if self.add_adapter:
            # 並行 Adapter 路徑
            mlp_adapter_output = self.mlp_adapter(x_norm)
            
            # 並行融合
            x = mlp_residual + mlp_output + mlp_adapter_output
            
            if collect_feats:
                collected_features['mlp_adapter'] = mlp_adapter_output
        else:
            # 原始融合
            x = mlp_residual + mlp_output
...
class TextDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, xa: Tensor, kv_cache: Optional[dict] = None, 
        ...
        # --- START: Translation Dropout Logic ---
        
        # 1. 檢查是否提供了 xt_list (不為 None)
        if xt_list is not None:
            # 2. 如果在訓練模式 (self.training 為 True)
            if self.training and self.dropout_rate > 0:
                # 依據 dropout_rate 機率隨機丟棄
                if torch.rand(1) < self.dropout_rate:
                    xt_list = None  # 丟棄，設為 None
            
            # 3. 如果在評估/推論模式 (self.training 為 False)
            elif not self.training:
                xt_list = None  # 永遠丟棄

        # --- END: Translation Dropout Logic ---
