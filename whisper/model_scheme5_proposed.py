...
class TextDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, xa: Tensor, kv_cache: Optional[dict] = None, 
        ...
        if xt_list is not None:
            for xt in xt_list:
                # 1. Baseline 模式 (Self-Attn)
                if xt is None:
                    processed_xt_list.append(None)
                    continue

                # 2. 如果 BERT hidden size != n_state，要先投影 (Audio 1280=1280 會是 Identity)
                if xt.shape[-1] != x.shape[-1]:
                    xt = self.text_projection(xt)   # 768 -> 1280 (我們要利用這個!)

                # 3. [關鍵修改] 判斷是否加 Positional Embedding
                # 如果長度超過預設的 Text Context (448)，假設它是 Audio，跳過加 Pos Emb
                if xt.shape[1] <= self.positional_embedding.shape[0]:
                    xt = xt + self.positional_embedding[offset : offset + xt.shape[1]]
                else:
                    # Audio Features 已經在 Encoder 加過 Pos Emb 了，這裡不要重複加
                    pass
