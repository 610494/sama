...
class PredictionHead(nn.Module):
    """
    用於從 Audio Encoder 輸出 (xa) 預測翻譯摘要向量。
    """
    def __init__(self, n_audio_state: int, n_text_state: int, n_mlp_ratio: int = 2):
        """
        n_audio_state: Audio Encoder 的隱藏層維度 (n_state)
        n_text_state:  Text Decoder 的隱藏層維度 (n_state)
        """
        super().__init__()
        # 建立一個簡單的 MLP (多層感知器)
        n_mlp = n_audio_state * n_mlp_ratio
        self.mlp = nn.Sequential(
            Linear(n_audio_state, n_mlp),
            nn.GELU(),
            Linear(n_mlp, n_text_state)
        )
        self.ln = LayerNorm(n_audio_state)

    def forward(self, xa: Tensor) -> Tensor:
        """
        xa: Audio Encoder 輸出, shape [Batch, T_audio, D_audio_state]
        """
        # 1. 對 Encoder 輸出進行 LayerNorm
        xa = self.ln(xa)
        
        # 2. 對時間維度 (dim=1) 進行平均池化，以獲得一個固定的摘要向量
        # Shape: [Batch, T_audio, D_audio_state] -> [Batch, D_audio_state]
        xa_pooled = xa.mean(dim=1)
        
        # 3. 通過 MLP 進行預測
        # Shape: [Batch, D_audio_state] -> [Batch, D_text_state]
        predicted_summary = self.mlp(xa_pooled)
        
        # 4. 增加一個 "seq_len" 維度 (維度為 1)
        # Shape: [Batch, D_text_state] -> [Batch, 1, D_text_state]
        # 這是為了讓 Gated Cross-Attention 模組可以處理它
        return predicted_summary.unsqueeze(1)
...
class Whisper(nn.Module):
    def __init__(self, dims: ModelDimensions, dropout_rate: float,
        ...
        # --- START: 新增 PredictionHead ---
        if add_gated_x_attn != 0:
            print("Adding PredictionHead for Translation-Guided ASR (Scheme 3)")
