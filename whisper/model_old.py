...
class TextDecoder(nn.Module):
    def __init__(
        ...
        # [優化] 儲存 bert_dim 以便在 forward 中做安全檢查
        self.bert_hidden_size = bert_hidden_size

        # 1. Text Projection
        # 即使維度一樣，建議總是過一層 Identity，方便 forward 統一寫法
    ...
    def forward(self, x: Tensor, xa: Tensor, kv_cache: Optional[dict] = None, 
        ...
        if xt_list is not None:
            for xt in xt_list:
                ...
                if xt.shape[-1] == self.bert_hidden_size:
                    ...
                    # [修正] 只有當長度在 Decoder 支援的範圍內時，才加 Positional Embedding
                    # Scheme 5 (Text) 會進入這個 if
                    # Scheme 3 (Audio Sequence) 因為長度 (500) > n_ctx (448)，會跳過這個 if
                    if xt.shape[1] <= self.positional_embedding.shape[0]:
                        xt = xt + self.positional_embedding[: xt.shape[1]]

                    else:
                        # Scheme 3 OT: 來自 Audio Encoder 的特徵已經有 Positional Info 了，不需要再加
                        pass 
        ...
        for block in self.blocks:
            ...
            if return_adapter_out and collected_features:
                last_collected_features = collected_features
        ...
        if return_hidden:
            ...
            return logits, last_collected_features
        else:
            return logits
